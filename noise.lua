...
local function getValueCosine(X1, X2, Y1, Y2, X3)
	--if (X3 < X1 or X3 > X2) then error("Invalid X3!") end
	--if (X1 == X2) then error("X1 shouldn't be X2!") end
	--if (Y1 == Y2) then error("Y1 shouldn't be Y2!") end
	local Y3 = (1-math.cos(math.pi/(X1-X2) * (X3-X1))) / 2 * (Y2-Y1) + Y1--Y3 = (Y2 - Y1) / 2 * (1 - math.cos(math.pi * (X3 - X1) / (X2 - X1))) + Y1
...
local function compressNoiseLayers(noiseLayers)
	noise = {}
	noiseSize = table.getn(noiseLayers[1])
...
function createNoise(size, x, y, seed, smoothness)
	...
	while (layerWidth <= size / (2 ^ smoothness)) do
		...
		table.insert(noiseLayers, layer)
